...
$tablesToCheck = [
    ...
    'expenses' => ['id', 'title', 'amount', 'category', 'description', 'status'],
    'check_ins' => ['id', 'user_id', 'today', 'currentTime', 'reason', 'is_supplementary']
];
...
try {
    ...
    $pdo->exec("INSERT INTO contacts (id, full_name, status, tenant_id) VALUES (9999, 'Khách hàng Test Bể Cọc 1', 'Đã Gặp', 1) ON DUPLICATE KEY UPDATE status='Đã Gặp'");
    $pdo->exec("INSERT INTO deposits (id, contact_id, amount, status, tenant_id) VALUES (9999, 9999, 50000000, 'pending', 1) ON DUPLICATE KEY UPDATE status='pending'");
    
    // Giả lập hệ thống tự chuyển trạng thái KHTN sang "Đặt Cọc" khi lập phiếu cọc
    $pdo->exec("UPDATE contacts SET status = 'Đặt Cọc' WHERE id = 9999");
    assertDbField($conn, 'contacts', 'status', 'id = 9999', 'Đặt Cọc', "Bước 1: Chuyển trạng thái sang Đặt Cọc sau khi tạo phiếu cọc");
    ...
    $qRevenue = $pdo->prepare("SELECT SUM(amount) as paid FROM deposits WHERE contact_id = ? AND status = 'approved'");
    $qRevenue->execute([9999]);
    $revenue = $qRevenue->fetch()['paid'] ?? 0;
    
    // Logic nghiệp vụ: Nếu doanh thu = 0, tụt trạng thái về "Đã Gặp" (hoặc Booking)
    if ($revenue == 0) {
        $pdo->exec("UPDATE contacts SET status = 'Đã Gặp' WHERE id = 9999");
    }
    
    assertDbField($conn, 'contacts', 'status', 'id = 9999', 'Đã Gặp', "Bước 2: Tụt trạng thái KHTN về Đã Gặp do cọc bể & chưa phát sinh doanh thu thực tế");
} catch (\Throwable $e) {
...
try {
    $pdo->exec("INSERT INTO contacts (id, full_name, status, tenant_id) VALUES (8888, 'Khách hàng Test Bể Cọc 2', 'Đã Gặp', 1) ON DUPLICATE KEY UPDATE status='Đã Gặp'");
    // Đã thanh toán đợt 1 (status = 'approved') -> đã thu tiền
    $pdo->exec("INSERT INTO deposits (id, contact_id, amount, status, tenant_id) VALUES (8888, 8888, 20000000, 'approved', 1) ON DUPLICATE KEY UPDATE status='approved'");
    
    $pdo->exec("UPDATE contacts SET status = 'Đặt Cọc' WHERE id = 8888");
    
    // Giả lập cọc bể nhưng đã đóng đợt 1 -> Giữ nguyên Đặt Cọc
    $qRevenue2 = $pdo->prepare("SELECT SUM(amount) as paid FROM deposits WHERE contact_id = ? AND status = 'approved'");
    $qRevenue2->execute([8888]);
    $revenue2 = $qRevenue2->fetch()['paid'] ?? 0;
    
    if ($revenue2 == 0) {
        $pdo->exec("UPDATE contacts SET status = 'Đã Gặp' WHERE id = 8888");
    } else {
        // Giữ nguyên Đặt Cọc (Không hạ cấp do đã phát sinh dòng tiền thực)
        $pdo->exec("UPDATE contacts SET status = 'Đặt Cọc' WHERE id = 8888");
    }
    
    assertDbField($conn, 'contacts', 'status', 'id = 8888', 'Đặt Cọc', "Bước 2: Giữ nguyên trạng thái Đặt Cọc do đã phát sinh doanh thu thực tế (Đã đóng tiền thực)");
} catch (\Throwable $e) {
